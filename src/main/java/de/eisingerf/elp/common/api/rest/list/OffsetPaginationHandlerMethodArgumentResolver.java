...
public class OffsetPaginationHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
	...
	public OffsetPaginationParameter resolveArgument(@Nonnull MethodParameter parameter, ModelAndViewContainer mavContainer, @Nonnull NativeWebRequest webRequest, WebDataBinderFactory binderFactory) {
		...
		var parameterAnnotations = parameter.getParameterType().getConstructors()[0].getParameterAnnotations();
		var offsetParamAnnotation = (RequestParam) Arrays.stream(parameterAnnotations[0]).filter(annotation -> RequestParam.class.equals(annotation.annotationType())).findFirst().orElseThrow();
		var limitParamAnnotation = (RequestParam) Arrays.stream(parameterAnnotations[1]).filter(annotation -> RequestParam.class.equals(annotation.annotationType())).findFirst().orElseThrow();

		String offsetValue = Optional.ofNullable(webRequest.getParameter(offsetParamAnnotation.name())).orElse(offsetParamAnnotation.defaultValue());
		String limitValue = Optional.ofNullable(webRequest.getParameter(limitParamAnnotation.name())).orElse(limitParamAnnotation.defaultValue());
